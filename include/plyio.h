//
// Created by Zachary on 2023/9/22.
//

#ifndef PLYFASTLOADER_PLYIO_H
#define PLYFASTLOADER_PLYIO_H
#include <array>
#include <cctype>
#include <fstream>
#include <iostream>
 ...
 */
    ...
    public:
        ...
        virtual void readNext(char* buff) = 0;
        ...
        virtual void readNextBigEndian(char* buff) = 0;
        ...
        /**
         * @brief Size of an element entry
         * @return
         */
        virtual size_t eleSize() = 0;
    };
 ...
 */
    ...
    public:
         ...
         * @param name_
         */
        TypedProperty(const std::string& name_) : Property(name_), data(std::make_shared<std::vector<T>>()) {
        ...
        TypedProperty(const std::string& name_, size_t eleSize) : Property(name_), elementSize(eleSize), data(std::make_shared<std::vector<T>>()) {
            if (typeName<T>() == "unknown") {
                // TODO should really be a compile-time error
                throw std::runtime_error("Attempted property type does not match any type defined by the .ply format.");
            }
         ...
         * @param data_
         */
        TypedProperty(const std::string& name_, const std::vector<T>& data_) : Property(name_), data(std::make_shared<std::vector<T>>(data_)) {
            if (typeName<T>() == "unknown") {
                throw std::runtime_error("Attempted property type does not match any type defined by the .ply format.");
            }
        };
        ...
        virtual void readNext(char* buff) override {
            data->emplace_back();
            memcpy_s((char*)&data->back(), sizeof(T), buff, sizeof(T));
        }
        ...
        virtual void readNextBigEndian(char* buff) override {
            data->emplace_back();
            memcpy_s((char*)&data->back(), sizeof(T), buff, sizeof(T));
            data->back() = swapEndian(data->back());
        }
         ...
         * @return
         */
        virtual size_t eleSize() override {
            return elementSize;
        }
 ...
 */
    ...
    public:
         ...
         * @return
         */
        virtual size_t eleSize() override { return 0; }
    ...
    inline std::unique_ptr<Property> createPropertyWithType(const std::string& name, const std::string& typeStr,
        ...
        if (typeStr == "uchar" || typeStr == "uint8") {
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<uint8_t>(name, 1));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<uint16_t>(name, 2));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<uint32_t>(name, 4));
            }
        }
        ...
        if (typeStr == "char" || typeStr == "int8") {
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<int8_t>(name, 1));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<int16_t>(name, 2));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<int32_t>(name, 4));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<float>(name, 4));
            }
        }
            ...
            if (isList) {
                ...
                return std::unique_ptr<Property>(new TypedProperty<double>(name, 8));
            }
        }
        ...
        void validate() {
            ...
            for (size_t iP = 0; iP < properties.size(); iP++) {
                if (properties[iP]->size() != count) {
                    throw std::runtime_error("Ply validate: property has wrong size. " + name + "->" + properties[iP]->name + " " + std::to_string(properties[iP]->size()) +
                                             " does not match element size " + std::to_string(count));
                }
            }
        }
 ...
 */
    ...
    public:
        ...
        static size_t prepareSplit(const std::string& filename, size_t splitSize, bool verbose = false) {//size : bytes
            using std::cout;
            using std::endl;
            using std::string;
            using std::vector;

            if (verbose) cout << "PLY prepare split: Reading ply file: " << filename << endl;

            std::ifstream inStream(filename, std::ios::binary);
            if (inStream.fail()) {
                throw std::runtime_error("PLY prepare split: Could not open file " + filename);
            }

            const SplitParams& splitParams = parseHeaderForSplit(inStream, splitSize, verbose);

            if (verbose) {
                cout << "  - Finished prepare split" << endl;
            }
            return splitParams.blockCount;
        }

        static void writeToPly(const std::string &path, PCT::PointCloudData& inputData) {
            namespace fs = std::experimental::filesystem::v1;
            try {
                PLYData plyOut;
                plyOut.addElement("vertex", inputData.count());
                Element& elemVertexOut = plyOut.getElement("vertex");
                for (int i=0; i<inputData.getSpec().size(); i++) {
                    const std::pair<std::string,std::string>& pair = inputData.getSpec()[i];
                    const std::string& typeName = pair.first;
                    const std::string& typeStr = pair.second;
                    if (typeStr == "uchar" || typeStr == "uint8") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<uint8_t>>(inputData.getData()[i])));
                    } else if (typeStr == "ushort" || typeStr == "uint16") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<uint16_t>>(inputData.getData()[i])));
                    } else if (typeStr == "uint" || typeStr == "uint32") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<uint32_t>>(inputData.getData()[i])));
                    } else if (typeStr == "char" || typeStr == "int8") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<int8_t>>(inputData.getData()[i])));
                    } else if (typeStr == "short" || typeStr == "int16") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<int16_t>>(inputData.getData()[i])));
                    } else if (typeStr == "int" || typeStr == "int32") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<int32_t>>(inputData.getData()[i])));
                    } else if (typeStr == "float" || typeStr == "float32") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<float>>(inputData.getData()[i])));
                    } else if (typeStr == "double" || typeStr == "float64") {
                        elemVertexOut.addProperty(typeName, *(std::static_pointer_cast<std::vector<double>>(inputData.getData()[i])));
                    }
                }
                if (fs::exists(path)) {
                    fs::remove(path);
                }
                plyOut.write(path, DataFormat::Binary);
            } catch (std::exception& exception) {
                std::cout << "err:" << exception.what() << std::endl;
            }
        }

        PLYData(const std::string& filename, size_t splitSize, size_t blockIndex, bool verbose = false) {
            vertexSplitBlockIndex = blockIndex;
            std::ifstream inStream(filename, std::ios::binary);
            if (inStream.fail()) {
                throw std::runtime_error("PLY prepare split: Could not open file " + filename);
            }
            const SplitParams& splitParams = parseHeaderForSplit(inStream, splitSize, verbose);
            vertexSplitBlockSize = splitParams.blockSize;
            vertexSplitBlockCount = splitParams.blockCount;
        ...
        void init(const std::string& filename, bool verbose = false) {
            ...
            if (verbose) {
                cout << "  - Finished parsing file, time = " << end-start << endl;
            }
        }
            ...
            for (size_t iE = 0; iE < elements.size(); iE++) {
                ...
                for (size_t jE = iE + 1; jE < elements.size(); jE++) {
                    if (elements[iE].name == elements[jE].name) {
                        throw std::runtime_error("Ply validate: duplicate element name " + elements[iE].name);
                    }
                }
            }
        ...
        /**
         * @brief Append this data to a .ply file.
         *
         * @param filename The file to write to.
         * @param format The format to use (binary or ascii?)
         */
        void append(const std::string& filename, DataFormat format = DataFormat::ASCII) {
            outputDataFormat = format;
            ...
            if (!inStream.good()) {
                throw std::runtime_error("Ply writer: Could not open input file " + filename + " for reading");
            }
            // Open stream for writing
            const std::string& tempFilename = filename + ".temp";
            std::ofstream outStream(tempFilename, std::ios::out | std::ios::binary);
            if (!outStream.good()) {
                throw std::runtime_error("Ply writer: Could not open output file " + filename + " for writing");
            }

            appendPLY(inStream, outStream);
            inStream.close();
            outStream.close();

            try {
                namespace fs = std::experimental::filesystem::v1;
                fs::path oldPath(filename);
                if (fs::remove(oldPath)) {
                    fs::rename(fs::path(tempFilename), oldPath);
                }
            } catch (std::exception& exception) {
                std::cout << "err:" << exception.what() << std::endl;
            }
        ...
        void getVertexData(PointCloudData& pointCloudData) {
            pointCloudData.reset();
            std::vector<std::pair<std::string,std::string>>& spec = pointCloudData.getSpec();
            std::vector<std::shared_ptr<void>>& data = pointCloudData.getData();

            Element& eleVertex = getElement("vertex");
            for (std::unique_ptr<Property>& prop : eleVertex.properties) {
                std::string typeStr = prop->propertyTypeName();

                spec.emplace_back(std::pair<std::string,std::string>({prop->name, typeStr}));
                if (typeStr == "uchar" || typeStr == "uint8") {
                    auto castedProp = dynamic_cast<TypedProperty<uint8_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    }
                } else if (typeStr == "ushort" || typeStr == "uint16") {
                    auto castedProp = dynamic_cast<TypedProperty<uint16_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<uint32_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<int8_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<int16_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<int32_t>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<float>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
                    ...
                    auto castedProp = dynamic_cast<TypedProperty<double>*>(prop.get());
                    if (castedProp) {
                        data.emplace_back(castedProp->data);
                    } else {
                        throw std::runtime_error("cast target type failed:"+typeStr);
            ...
            pointCloudData.notifySpecChanged();
            pointCloudData.notifyDataContainersChanged();
        }
    ...
    private:
        ...
        size_t vertexSplitBlockSize;//the vertex count in a block
        size_t vertexSplitBlockCount;//the count of all the block in the element

        size_t vertexSplitBlockIndex = -1;//the index of vertex block should be read;
        // === Reading ===
        ...
        /**
         * reading and parse header for prepare split
         * @param inStream
         * @param splitSize
         * @param verbose
         */
        static SplitParams parseHeaderForSplit(std::istream& inStream, size_t splitSize, bool verbose) {
...
//                    inputDataFormat = DataFormat::BinaryBigEndian;
            ...
            if (!elements.empty()) {
                for (Element& element : elements) {
                    if (element.name == "vertex" && !element.properties.empty()) {
                        if (verbose) {
                            cout << "PLY parser: found vertex element" << endl;
                        }
                        size_t eleTotalSize = 0;
                        for (auto & prop : element.properties) {
                            eleTotalSize += prop->eleSize();
                        }

                        if (verbose) {
                            cout << "PLY parser: calculated size of an element:" << eleTotalSize << endl;
                        }

                        if (eleTotalSize > 0) {
                            splitParams.blockSize = splitSize/eleTotalSize;
                            splitParams.blockCount = element.count/splitParams.blockSize + (element.count%splitParams.blockSize == 0 ? 0 : 1);

                        }

                        if (verbose) {
                            cout << "PLY parser: vertexSplitBlockSize:" << splitParams.blockSize << ", vertexSplitBlockCount:" << splitParams.blockCount << endl;
                        }
        ...
        void parseASCII(std::istream& inStream, bool verbose) {
            ...
            for (Element& elem : elements) {
                ...
                if (vertexSplitBlockIndex >= 0 && vertexSplitBlockCount > 0 && vertexSplitBlockSize > 0 && vertexSplitBlockIndex < vertexSplitBlockCount) {
                    if (elem.name == "vertex") {
                        size_t startIndex = vertexSplitBlockSize * vertexSplitBlockIndex;
                        size_t endIndex = startIndex + vertexSplitBlockSize;//exclude
                        if (endIndex > elem.count) {
                            endIndex = elem.count;
                        }
                        size_t originCount = elem.count;
                        elem.count = endIndex - startIndex;
                        ...
                        for (size_t i = 0; i < endIndex; i++) {
                            ...
                            if (i < startIndex) {//skip lines until startIndex
                                continue;
                            }
            ...
            for (Element& elem : elements) {
                ...
                if (vertexSplitBlockIndex >= 0 && vertexSplitBlockCount > 0 && vertexSplitBlockSize > 0 && vertexSplitBlockIndex < vertexSplitBlockCount) {
                    if (elem.name == "vertex") {
                        ...
                        elem.count = endIndex - startIndex;
                        size_t vertexEleSize = 0;

                        for (auto & prop : elem.properties) {
                            prop->reserve(elem.count);
                            vertexEleSize += prop->eleSize();
                        }

                        if (verbose) {
                            std::cout << "  - StartIndex: " << startIndex << ", endIndex:" << endIndex << ", vertexEleSize:" << vertexEleSize << std::endl;
                        }

                        inStream.seekg(vertexEleSize * startIndex, std::ios_base::cur);

                        try {
                            size_t len = vertexEleSize*elem.count;
                            auto * buff = new char[len+1];
                            inStream.read(buff, len);

                            size_t offset = 0;
                            for (size_t iEntry = 0; iEntry < elem.count; iEntry++) {
                                for (size_t iP = 0; iP < elem.properties.size(); iP++) {
                                    elem.properties[iP]->readNext(buff+offset);
                                    offset += elem.properties[iP]->eleSize();
                                }
                            }
                            delete[] buff;
                        } catch (std::exception& exception) {
                        ...
                        for (size_t iP = 0; iP < elem.properties.size(); iP++) {
                            size_t eleSize = elem.properties[iP]->eleSize();
                            inStream.seekg(eleSize*elem.count, std::ios_base::cur);
                        }
                        elem.count = 0;
                    }
                } else {
                ...
                if (vertexSplitBlockIndex >= 0 && vertexSplitBlockCount > 0 && vertexSplitBlockSize > 0 && vertexSplitBlockIndex < vertexSplitBlockCount) {
                    if (elem.name == "vertex") {
                        ...
                        try {
                            ...
                            for (size_t iEntry = 0; iEntry < elem.count; iEntry++) {
                                for (size_t iP = 0; iP < elem.properties.size(); iP++) {
                                    elem.properties[iP]->readNextBigEndian(buff+offset);
                                    offset += elem.properties[iP]->eleSize();
                                }
                            }
                            delete[] buff;
        ...
        /**
         * @brief Append a PLY file with input stream into an output stream
         *
         * @param outStream
         */
        void appendPLY(std::istream& inStream, std::ostream& outStream) {
            // Write header
            appendHeader(inStream, outStream);
        ...
        /**
         * @brief Append a header with input stream into an output stream
         *
         * @param outStream
         */
        void appendHeader(std::istream& inStream, std::ostream& outStream) {
            using std::string;
            using std::cout;
            using std::endl;
            using std::vector;
            ...
            while (inStream.good()) {
                ...
                if (startsWith(line, "element")) {
                    vector<string> tokens = tokenSplit(line);
                    if (tokens.size() != 3) throw std::runtime_error("PLY append: Invalid element line");
                    string name = tokens[1];
                    size_t count;
                    std::istringstream iss(tokens[2]);
                    iss >> count;
